...
Функция СформироватьЗаголовокДокумента(Шапка, знач НазваниеДокумента = "") Экспорт
	...
	Если Шапка.Свойство("НомерКИС") И ЗначениеЗаполнено(Шапка.НомерКИС) Тогда 
		НомерНаПечать = Шапка.НомерКИС;
	Иначе	
		НомерНаПечать = Шапка.Номер;
	КонецЕсли;	
	Если Шапка.Свойство("ДатаКИС") И ЗначениеЗаполнено(Шапка.ДатаКИС) Тогда 
		ДатаНаПечать = Шапка.ДатаКИС;
	Иначе	
		ДатаНаПечать  = Шапка.Дата;
	КонецЕсли;	
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = '%1 № %2 от %3'"),
		НазваниеДокумента, НомерНаПечать, Формат(ДатаНаПечать, "ДФ='дд ММММ гггг'"));
...
Процедура УстановитьПараметрДинамическогоСписка(Список, ИмяПараметра, Значение, Использование = Истина) Экспорт
	...
	Если Использование Тогда
		Список.КомпоновщикНастроек.Настройки.ПараметрыДанных.УстановитьЗначениеПараметра(ИмяПараметра, Значение);
	Иначе
		ЗначениеПараметра = Список.КомпоновщикНастроек.Настройки.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных(ИмяПараметра));
