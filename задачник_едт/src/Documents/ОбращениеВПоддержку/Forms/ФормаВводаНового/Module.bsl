...
Процедура ВидОбращенияПриИзменении(Элемент)
	ИзменитьТипОбращениеОт(ВидОбращения);
	ПриИзмененииВидОбращенияНаСерврере();
	ПриИзмененииОбращениеОтНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьТипОбращениеОт(ВидОбращения)
	Если ВидОбращения = ОбщийМодульСервер.ПолучитьВидыОбращений("Внутренние") Тогда
		ТипСтр = "СправочникСсылка.Отделы";
	Иначе	
		ТипСтр = "СправочникСсылка.Контрагенты";
	КонецЕсли;	
	ОписаниеТипа = Новый ОписаниеТипов(ТипСтр);
	Элементы.ОбращениеОт.ОграничениеТипа = ОписаниеТипа;
	Элементы.ОбращениеОт.ВыбиратьТип = Ложь;	
КонецПроцедуры	
	
&НаСервере
Процедура ПриИзмененииВидОбращенияНаСерврере()
	Если ТипЗнч(Объект.ОбращениеОт) =  Тип("СправочникСсылка.Контрагенты") Тогда
		Объект.ОбращениеОт = Справочники.Контрагенты.ПустаяСсылка();
	Иначе
		Объект.ОбращениеОт = Справочники.Отделы.ПустаяСсылка();
	КонецЕсли;	
КонецПроцедуры
...
Процедура ПриИзмененииОбращениеОтНаСервере()
	...
	Объект.КонтактноеЛицо = ""; Объект.Телефон = "";
	Если ТипЗнч(Объект.ОбращениеОт) =  Тип("СправочникСсылка.Контрагенты") Тогда
		Объект.КонтактноеЛицо = Объект.ОбращениеОт.КонтактноеЛицо;
		Объект.Телефон = Объект.ОбращениеОт.Телефон;
	Иначе
		ЗаполнитьКонтактыПоОтделу();
	КонецЕсли;		
