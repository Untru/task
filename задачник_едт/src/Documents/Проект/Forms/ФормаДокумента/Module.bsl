...
&НаСервере
Процедура УстановитьОтборСписокаЗадачИДопЗатрат(ВыбранныйЭтап)
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокЗадачЭтапа.Отбор, "ЭтапПроекта", ВыбранныйЭтап);
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(СписокДопЗатрат.Отбор, "ЭтапПроекта", ВыбранныйЭтап);
...
Процедура СписокЭтаповПриАктивизацииСтроки(Элемент)
	...
	КонецЕсли;
	УстановитьОтборСписокаЗадачИДопЗатрат(ВыбранныйЭтап);
КонецПроцедуры
...
&НаКлиенте
Процедура ДобавитьЭтапПоддержки(Команда)
	//TODO: Здесь нужно описать открытие формы нового документа Этап, с наименованием - "Поддержка за ММММ.ГГГГ"
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписанЭтапПроекта" И Объект.Ссылка = Параметр Тогда 
		УстановитьСумму();
	ИначеЕсли (ИмяСобытия = "ЗаписанДокументДополнительныхЗатрат" ИЛИ ИмяСобытия = "ЗаписанаЗадача") И Объект.Ссылка = Параметр Тогда
