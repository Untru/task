...
Процедура ЗаполнитьТЧНастройкиКолонок()Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	узСтатусыЗадачи.ТекущийСтатус КАК Статус,
	|	Истина КАК Видимость,
	|	узСтатусыЗадачи.Заголовок КАК ИмяПредопределенныхДанных
	|ИЗ
	|	Документ.ОбращениеВПоддержку КАК узСтатусыЗадачи
	|
	|УПОРЯДОЧИТЬ ПО
	|	узСтатусыЗадачи.Номер";
