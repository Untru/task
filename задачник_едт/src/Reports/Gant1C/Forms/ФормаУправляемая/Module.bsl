...
Процедура ОбновитьЗаголовок();
	ЭтаФорма.Заголовок = НСтр("ru = 'Gant 1C - '") + ?(ЗначениеЗаполнено(ВыходнойФайлКраткоеИмя), ВыходнойФайлКраткоеИмя, "Новый.pmp");
...
Процедура ИмпортироватьРесурсыИзФайла(Команда)
	...
	Диалог.Показать(Новый ОписаниеОповещения("ИмпортироватьРесурсыИзФайлаЗавершение", ЭтаФорма, Новый Структура("Диалог", Диалог)));
КонецПроцедуры
...
Процедура ОткрытьФормуЗадачи(Знач РедактированиеРесурсов = ложь)
	...
	Попытка
		ФормаЗадачи = ПолучитьФорму("ВнешнийОтчет.Gant1C.Форма.ФормаЗадачиУправляемая", Новый Структура("Ключ, МассивНазванийЗадач", Объект, МассивНазванийЗадач), ЭтаФорма);
	Исключение
		ФормаЗадачи = ПолучитьФорму("Отчет.Gant1C.Форма.ФормаЗадачиУправляемая", Новый Структура("Ключ, МассивНазванийЗадач", Объект, МассивНазванийЗадач), ЭтаФорма);
	КонецПопытки;
	...
	ФормаЗадачи.ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ПрименитьИзмененияЗадачиВФорме", ЭтаФорма, Новый Структура("НомерСтрокиЗадачи, НомерСтрокиЗадачиКонец, ФормаЗадачи", НомерСтрокиЗадачи, НомерСтрокиЗадачиКонец, ФормаЗадачи));
	ФормаЗадачи.Открыть();	
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если  ИмяСобытия = "Запись_Файл" и Параметр.Событие = "СозданФайл" и Параметр.Владелец = Объект.Проект и ЗначениеЗаполнено(ИмяСоздаваемогоФайлаБСП) Тогда
		Если ИмяСоздаваемогоФайлаБСП = Строка(Параметр.Файл) Тогда
			...
			Выполнить("РаботаСФайламиСлужебныйКлиент.ЗанятьСОповещением(Неопределено, Параметр.Файл,УникальныйИдентификатор)");			
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновлениеОтображения()
	...
	Попытка
		Элементы.НадписьИсполнительПоУмолчанию.Заголовок = НСтр("ru = 'Ответственный по умолчанию (для новых задач): '") + Строка(?(ЗначениеЗаполнено(Объект.Проект.Исполнитель), Объект.Проект.Исполнитель, ТекущийПользовательСтрока));
	Исключение
...
Процедура ЗаполнитьПроектПоЗадачамИБ(Команда)
	...
	Если МодифицированностьПроекта и НеСпрашиватьОСохраненииПроектаВФайл <> Истина Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПроектПоЗадачамИБЗавершение", ЭтаФорма), ?(ЗначениеЗаполнено(ВыходнойФайлКраткоеИмя), НСтр("ru = 'Сохранить изменения проекта в """"'") + ВыходнойФайлКраткоеИмя+"""", НСтр("ru = 'Сохранить проект в файл'"))+"?", РежимДиалогаВопрос.ДаНетОтмена);
        Возврат;
	КонецЕсли;
	...
	Если НовыйФайл или Не ЗначениеЗаполнено(ВыходнойФайл) или ЗначениеЗаполнено(СсылкаНаФайлБСП) Тогда
		...
		Если КонфигурацияСодержитБСП() Тогда
			Если Не ЗначениеЗаполнено(СсылкаНаФайлБСП) или НовыйФайл = Истина Тогда	
				...
				ОписаниеОповещанияВыборИзМеню = Новый ОписаниеОповещения("СохранитьПроектВыборСпособа", ЭтаФорма,Новый Структура("СохранитьКакНовый", НовыйФайл));
...
Процедура СохранитьПроектВыборСпособа(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЭлементЗначения.Значение = "Сохранить на диске" Тогда
		...
		//Диалог.ПроверятьСуществованиеФайла = Истина;
		Диалог.Фильтр = НСтр("ru = 'Проект Gant 1C 2.0 (*.pmp2)|*.pmp2'");
		Диалог.Показать(Новый ОписаниеОповещения("СохранитьПроектЗавершение", ЭтаФорма, Новый Структура("Диалог", Диалог))); 
	ИначеЕсли ЭлементЗначения.Значение = "Сохранить в информационной базе" Тогда
		...
		ОбработчикРезультата = Новый ОписаниеОповещения("СохранитьПроектВИнформационнойБазеЗавершение", ЭтаФорма);
		
		Если ЗначениеЗаполнено(СсылкаНаФайлБСП) и Не ДополнительныеПараметры.СохранитьКакНовый = Истина Тогда
			...
			Попытка
				...
				Выполнить("ПараметрыОбновленияФайла = РаботаСФайламиСлужебныйКлиент.ПараметрыОбновленияФайла(ОбработчикРезультата, СсылкаНаФайлБСП,УникальныйИдентификатор)");
				...
				Выполнить("Обработчик = Новый ОписаниеОповещения(""СохранитьИзмененияФайлаПослеУстановкиРасширения"", РаботаСФайламиСлужебныйКлиент, ПараметрыОбновленияФайла)");
				Выполнить("ФайловыеФункцииСлужебныйКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Обработчик)");
			Исключение
				
				ТекстОшибки = ОписаниеОшибки();
				
				Выполнить("РаботаСФайламиСлужебныйКлиент.ОпубликоватьФайл(
		...
		Иначе	
			...
			ОписаниеОповещенияСохранитьНовыйПроектПослеУказанияИмениФайла = Новый ОписаниеОповещения("СохранитьНовыйПроектПослеУказанияИмениФайла", ЭтаФорма, Новый Структура("ВыходнойФайл", ВыходнойФайл));
			ПоказатьВводСтроки(ОписаниеОповещенияСохранитьНовыйПроектПослеУказанияИмениФайла, ИмяСоздаваемогоФайлаБСП, НСтр("ru = 'Gant 1C: Укажите имя файла в информационной базе'"),100,ложь);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура СохранитьНовыйПроектПослеУказанияИмениФайла(ВведеннаяСтрока, ДополнительныеПараметры) Экспорт
	...
	Выполнить("РаботаСФайламиСлужебныйКлиент.ДобавитьПослеВыбораРежимаСоздания(2, ПараметрыВыполнения)");
КонецПроцедуры
...
Процедура ЗагрузитьИзProject(Команда)
	...
	ДиалогОткрытияФайла.Показать(Новый ОписаниеОповещения("ЗагрузитьИзProjectЗавершение1", ЭтаФорма, Новый Структура("ДиалогОткрытияФайла, Текст", ДиалогОткрытияФайла, Текст)));
...
Процедура ЗагрузитьИзProjectЗавершение1(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ТабличноеПолеРесурсы.Количество() > 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗагрузитьИзProjectЗавершение", ЭтаФорма, Новый Структура("Текст", Текст)), НСтр("ru = 'Очистить перечень ресурсов?'"),РежимДиалогаВопрос.ДаНет);
		Возврат;
	Иначе
...
Процедура ОбновитьДиаграмму(пВерх = Неопределено, пНиз = Неопределено) Экспорт
	...
	ПолосаРегулированияШириныДиаграммыГанта = тдПроект.Область(,НомерКолонкиДиаграммыГанта, , НомерКолонкиДиаграммыГанта).ШиринаКолонки;
...
Процедура ВыборЗначенияФильтра(Знач НомерКолонки)
	...
	Если НомерКолонки = НомерКолонкиДлительность  Тогда
		...
		ОписаниеОповещанияВыборИзМеню = Новый ОписаниеОповещения("ОбработкаВыбораФильтраНаКлиенте", ЭтаФорма, Новый Структура("НомерКолонки", НомерКолонки));
...
Процедура ОбработкаВыбораФильтраНаКлиенте(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если НомерКолонки = НомерКолонкиДлительность и (ЭлементЗначения.Значение = 5 или ЭлементЗначения.Значение = 6) Тогда
		ОписаниеОповещанияВводаДительности = Новый ОписаниеОповещения("ОбработкаВыбораФильтраДлительностьНаКлиенте", ЭтаФорма, Новый Структура("НомерКолонки, Значение", НомерКолонки, ЭлементЗначения.Значение));
...
Процедура ОткрытьПроект(Команда)
	Если КонфигурацияСодержитБСП() Тогда
		...
		ОписаниеОповещанияВыборИзМеню = Новый ОписаниеОповещения("ОткрытьПроектВыборСпособа", ЭтаФорма);
...
Процедура ОткрытьПроектВыборСпособа(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если ЭлементЗначения.Значение = "Открыть файл на диске" Тогда
		...
		//Диалог.ПроверятьСуществованиеФайла = Истина;
		Диалог.Фильтр = НСтр("ru = 'Проект Gant 1C 2.0 (*.pmp2)|*.pmp2'");
		Диалог.Показать(Новый ОписаниеОповещения("ОткрытьПроектЗавершение", ЭтаФорма, Новый Структура("Диалог", Диалог)));
	ИначеЕсли ЭлементЗначения.Значение = "Открыть файл в информационной базе" Тогда
		...
		ФормаВыбораФайла = ПолучитьФорму("Справочник.Файлы.ФормаВыбора", Новый Структура("Отбор, ВладелецФайла", ЗначенияОтбора, Объект.Проект), ЭтаФорма);
				
		ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ОткрытьПроектОбработкаРезультатаОткрытияФормы", ЭтаФорма);
...
Процедура ОткрытьПроектОбработкаРезультатаОткрытияФормы(РезультатОткрытия, ДополнительныеПараметры) Экспорт
	...
	ПараметрыВыполнения.Вставить("УникальныйИдентификатор", УникальныйИдентификатор);
	ПараметрыВыполнения.Вставить("ОбработчикРезультата",  Новый ОписаниеОповещения("ОткрытьПроектПроверкаПолученияФайла", ЭтаФорма));
	
	ТекстОшибки = "";
	ДанныеФайлаПолученыИОнЗанят = Ложь;
	Выполнить("ДанныеФайлаПолученыИОнЗанят = РаботаСФайламиСлужебныйВызовСервера.ПолучитьДанныеФайлаИЗанятьФайл(
	...
	Если Не ДанныеФайлаПолученыИОнЗанят Тогда // Если занять нельзя, тогда выводится сообщение об ошибке.
		Попытка
			Выполнить("РаботаСФайламиСлужебныйВызовСервера.ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстОшибки, Ложь)");
		Исключение
			Выполнить("РаботаСФайламиСлужебныйКлиент.ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстОшибки, Ложь)");
		КонецПопытки;
	...
	ВременныйПутьКФайлу = "";
	Выполнить("РаботаСФайламиСлужебныйКлиент.ПолучитьФайлВерсииВРабочийКаталог(
																		|Новый ОписаниеОповещения(""ОткрытьПроектПроверкаПолученияФайла"", ЭтаФорма),
																		|ПараметрыВыполнения.ДанныеФайла,
																		|ВременныйПутьКФайлу,
																		|УникальныйИдентификатор,
																		|Неопределено)");
...
Процедура ОсвободитьФайл(ОсвобождаемыйФайл)
	...
	ОбработчикРезультата = Новый ОписаниеОповещения("ОсвободитьФайлЗавершение", ЭтаФорма);
...
Процедура НастройкиПроекта(Команда)
	...
	Попытка
		ФормаНастройкиПроектаУправляемая =  ПолучитьФорму("ВнешнийОтчет.Gant1C.Форма.ФормаНастройкиПроектаУправляемая", Новый Структура("Ключ", Объект), ЭтаФорма);
	Исключение
		ФормаНастройкиПроектаУправляемая =  ПолучитьФорму("Отчет.Gant1C.Форма.ФормаНастройкиПроектаУправляемая", Новый Структура("Ключ", Объект), ЭтаФорма);
	КонецПопытки;
	
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("ПрименитьИзмененияНастроекПроекта", ЭтаФорма, Новый Структура("ФормаНастройкиПроектаУправляемая", ФормаНастройкиПроектаУправляемая));
...
Процедура НастройкаВидимостиКолонок(Команда)
	Попытка
		ФормаНастройкаВидимостиКолонокУправляемая = ПолучитьФорму("ВнешнийОтчет.Gant1C.Форма.НастройкаВидимостиКолонокУправляемая", Новый Структура("Ключ", Объект), ЭтаФорма);
	Исключение
		ФормаНастройкаВидимостиКолонокУправляемая = ПолучитьФорму("Отчет.Gant1C.Форма.НастройкаВидимостиКолонокУправляемая", Новый Структура("Ключ", Объект), ЭтаФорма);
	КонецПопытки;
	
	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения("НастройкаВидимостиКолонокОбработкаРезультатаОткрытияФормы", ЭтаФорма, Новый Структура("ФормаНастройкаВидимостиКолонокУправляемая", ФормаНастройкаВидимостиКолонокУправляемая));
...
Процедура Плюс(Команда)
	...
	НачатьЗапускПриложения(Новый ОписаниеОповещения("Пустышка", ЭтаФорма), СсылкаПлюса);
...
Процедура Минус(Команда)
	ОписаниеОповещенияВопроса = Новый ОписаниеОповещения("СтавимМинутНаИнфостарте", ЭтаФорма);
...
Процедура СтавимМинутНаИнфостарте(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Закрыть();
	Иначе
		ОписаниеОповещенияВопроса = Новый ОписаниеОповещения("СтавимПлюсНаИнфостарте", ЭтаФорма);
...
Процедура ОПрограмме(Команда)
	Попытка
		Результат = ОткрытьФорму("ВнешнийОтчет.Gant1C.Форма.ОПрограммеУправляема", Новый Структура("Ключ", Объект), ЭтаФорма);
	Исключение
		Результат = ОткрытьФорму("Отчет.Gant1C.Форма.ОПрограммеУправляема", Новый Структура("Ключ", Объект), ЭтаФорма);
	КонецПопытки;
КонецПроцедуры
...
Процедура ПолосаРегулированияШириныДиаграммыГантаПриИзменении(Элемент)
	Диаграмма   = тдПроект.Рисунки.получить(0).Объект;
	Диаграмма.Обновление = Ложь;
	тдПроект.Область(, НомерКолонкиДиаграммыГанта, ,НомерКолонкиДиаграммыГанта).ШиринаКолонки = ПолосаРегулированияШириныДиаграммыГанта;
	Диаграмма.Обновление = Истина;
КонецПроцедуры

&НаКлиенте
Процедура Справка(Команда)
	НачатьЗапускПриложения(Новый ОписаниеОповещения("Пустышка", ЭтаФорма), "https://dl.dropboxusercontent.com/u/16738832/Gant1C/Gant1C2_2.0_1_16.pdf");
КонецПроцедуры
...
Процедура НовыйПроект(Команда)
	Попытка
		ОткрытьФорму("ВнешнийОтчет.Gant1C.Форма.ФормаУправляемая", , , Истина);
	Исключение
		ОткрытьФорму("Отчет.Gant1C.Форма.ФормаУправляемая", , , Истина);
	КонецПопытки;
КонецПроцедуры
