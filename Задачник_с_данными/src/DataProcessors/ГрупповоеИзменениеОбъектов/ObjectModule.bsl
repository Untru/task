...
Процедура ЗаполнитьТаблицуОперацийСОбъектом() Экспорт
	...
	// если номенклатура изменяется, то
	// дополняем список операций
	
	Если ОбъектМетаданных.Имя = "усНоменклатура" Тогда
		
		// заполнение списка единиц измерений
		НоваяОперация = Операции.Добавить();
		НоваяОперация.ВидОперации 				= 4; // Вид операции = 4 - особая обработка реквизита.
		НоваяОперация.ВыборГруппИЭлементов 		= "Элементы";
		НоваяОперация.ЗаблокированныйРеквизит 	= Ложь;
		Новаяоперация.Представление 			= "Заполнение единиц измерения по шаблону";
		НоваяОперация.Изменять 					= Ложь;
		НоваяОперация.ИмяРеквизита 				= "ЗаполнениеЕИ_шаблон";
		НоваяОперация.ДопустимыеТипы 			= "СправочникСсылка.усНоменклатура";
		
		// заполнение зон отбора
		НоваяОперация = Операции.Добавить();
		НоваяОперация.ВидОперации 				= 4; // Вид операции = 4 - особая обработка реквизита.
		НоваяОперация.ВыборГруппИЭлементов 		= "Элементы";
		НоваяОперация.ЗаблокированныйРеквизит 	= Ложь;
		Новаяоперация.Представление 			= "Заполнение зон отбора по шаблону";
		НоваяОперация.Изменять 					= Ложь;
		НоваяОперация.ИмяРеквизита 				= "ЗаполнениеЗО_шаблон";
		НоваяОперация.ДопустимыеТипы 			= "СправочникСсылка.усНоменклатура";
		...
		НоваяОперация.ВыборГруппИЭлементов 		= "Элементы";
		НоваяОперация.ЗаблокированныйРеквизит 	= Ложь;
		Новаяоперация.Представление 			= "Заполнение вместимости контейнеров по шаблону";
		НоваяОперация.Изменять 					= Ложь;
		НоваяОперация.ИмяРеквизита 				= "ЗаполнениеВК_шаблон";
		НоваяОперация.ДопустимыеТипы 			= "СправочникСсылка.усНоменклатура";
	КонецЕсли;
...
Функция ВыполнитьИзменениеОбъектов(ОбрабатываемыеОбъекты = Неопределено,
	...
	тбзУпаковок        = Неопределено;  
	тбзУпаковкиОбъекта = Неопределено;
	тбзЗоныОтбора      = Неопределено;
	тбзВместимсоть     = Неопределено;
	...
	Для Каждого Ссылка Из ОбрабатываемыеОбъекты Цикл
		...
		Для Каждого Операция Из ОперацииИзменения Цикл
			...
			ИначеЕсли Операция.ВидОперации = 4 Тогда // прочие действия.
